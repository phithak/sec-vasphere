...
def inject_sudo_user_to_image(qcow2_path, vm_ip, vm_user):
    """
    Injects possible network fixes into the image using virt-customize.
    """
    virt_customize_cmd = (
    ...
    print(f"[+] Injecting network fixes via virt-customize: {virt_customize_cmd}")
...
def create_new_target(name, ip):
    ...
    target_xml = ssh_openvas_cmd(
        f"<create_target><name>{name}</name><hosts>{ip}</hosts><port_list id='{port_list_id}'/></create_target>")
...
def main():
    ...
    for target in targets:
        ...
        try:
            ...
            target_id = create_new_target(unique_target_name, ip)
